/**
 * GeminiAdapterLive - Scoped live implementation for the Gemini Agent provider adapter.
 *
...
import {
  type CanUseTool,
  query,
  type Options as GeminiQueryOptions,
  type PermissionMode,
...
import { getGeminiModelCapabilities } from "./GeminiProvider.ts";
import {
...
type GeminiTextStreamKind = Extract<RuntimeContentStreamKind, "assistant_text" | "reasoning_text">;
type GeminiToolResultStreamKind = Extract<
...
interface GeminiSessionContext {
  ...
  readonly query: GeminiQueryRuntime;
...
interface GeminiQueryRuntime extends AsyncIterable<SDKMessage> {
...
export interface GeminiAdapterLiveOptions {
  readonly createQuery?: (input: {
    ...
    readonly options: GeminiQueryOptions;
  }) => GeminiQueryRuntime;
...
function isSyntheticGeminiThreadId(value: string): boolean {
  return value.startsWith("gemini-thread-");
}
...
function normalizeGeminiStreamMessages(cause: Cause.Cause<Error>): ReadonlyArray<string> {
...
function isGeminiInterruptedMessage(message: string): boolean {
...
function isGeminiInterruptedCause(cause: Cause.Cause<Error>): boolean {
  return (
    Cause.hasInterruptsOnly(cause) ||
    normalizeGeminiStreamMessages(cause).some(isGeminiInterruptedMessage)
  );
}

function messageFromGeminiStreamCause(cause: Cause.Cause<Error>, fallback: string): string {
  return normalizeGeminiStreamMessages(cause)[0] ?? fallback;
}

function interruptionMessageFromGeminiCause(cause: Cause.Cause<Error>): string {
  const message = messageFromGeminiStreamCause(cause, "Gemini runtime interrupted.");
  return isGeminiInterruptedMessage(message) ? "Gemini runtime interrupted." : message;
}
...
function maxGeminiContextWindowFromModelUsage(modelUsage: unknown): number | undefined {
...
function normalizeGeminiTokenUsage(
  usage: unknown,
...
function readGeminiResumeState(resumeCursor: unknown): GeminiResumeState | undefined {
  ...
  const threadId =
    threadIdCandidate && !isSyntheticGeminiThreadId(threadIdCandidate)
...
function buildPromptText(input: ProviderSendTurnInput): string {
  const rawEffort =
    input.modelSelection?.provider === "gemini" ? input.modelSelection.options?.effort : null;
  const geminiModel =
    input.modelSelection?.provider === "gemini" ? input.modelSelection.model : undefined;
  const caps = getGeminiModelCapabilities(geminiModel);
...
function buildGeminiImageContentBlock(input: {
...
const buildUserMessageEffect = Effect.fn("buildUserMessageEffect")(function* (
  ...
  for (const attachment of input.attachments ?? []) {
    ...
    if (!SUPPORTED_GEMINI_IMAGE_MIME_TYPES.has(attachment.mimeType)) {
      return yield* new ProviderAdapterRequestError({
        provider: PROVIDER,
        method: "turn/start",
        detail: `Unsupported Gemini image attachment type '${attachment.mimeType}'.`,
      });
    }
...
function streamKindFromDeltaType(deltaType: string): GeminiTextStreamKind {
  return deltaType.includes("thinking") ? "reasoning_text" : "assistant_text";
}

function nativeProviderRefs(
  _context: GeminiSessionContext,
  options?: {
...
function toolResultStreamKind(itemType: CanonicalItemType): GeminiToolResultStreamKind | undefined {
  switch (itemType) {
    case "command_execution":
...
function sdkNativeMethod(message: SDKMessage): string {
  ...
  if (subtype) {
    return `gemini/${message.type}/${subtype}`;
  }
    ...
    if (streamType) {
      const deltaType =
        streamType === "content_block_delta"
          ...
          : undefined;
      if (deltaType) {
        return `gemini/${message.type}/${streamType}/${deltaType}`;
      }
      return `gemini/${message.type}/${streamType}`;
    }
  }

  return `gemini/${message.type}`;
}
...
const makeGeminiAdapter = Effect.fn("makeGeminiAdapter")(function* (
  ...
  const createQuery =
    ...
    ((input: {
      readonly prompt: AsyncIterable<SDKUserMessage>;
      readonly options: GeminiQueryOptions;
    }) => query({ prompt: input.prompt, options: input.options }) as GeminiQueryRuntime);
  ...
  const snapshotThread = Effect.fn("snapshotThread")(function* (context: GeminiSessionContext) {
  ...
  const createSyntheticAssistantTextBlock = Effect.fn("createSyntheticAssistantTextBlock")(
    function* (context: GeminiSessionContext, fallbackText: string) {
      const turnState = context.turnState;
      if (!turnState) {
        return undefined;
      }
  ...
  )(function* (context: GeminiSessionContext, message: SDKMessage) {
    const turnState = context.turnState;
    if (!turnState) {
      return;
    }
  ...
  const completeTurn = Effect.fn("completeTurn")(function* (
    ...
    const resultContextWindow = maxGeminiContextWindowFromModelUsage(result?.modelUsage);
    ...
    const accumulatedSnapshot = normalizeGeminiTokenUsage(
      resultUsage,
  ...
  const handleStreamEvent = Effect.fn("handleStreamEvent")(function* (
    ...
    if (event.type === "content_block_delta") {
      if (
        ...
        yield* offerRuntimeEvent({
          ...
          raw: {
            source: "claude.sdk.message",
            method: "gemini/stream_event/content_block_delta",
            payload: message,
          },
        });
        return;
      ...
      if (event.delta.type === "input_json_delta") {
        ...
        yield* offerRuntimeEvent({
          ...
          raw: {
            source: "claude.sdk.message",
            method: "gemini/stream_event/content_block_delta/input_json_delta",
            payload: message,
          },
        });
      }
    ...
    if (event.type === "content_block_start") {
      ...
      yield* offerRuntimeEvent({
        ...
        raw: {
          source: "claude.sdk.message",
          method: "gemini/stream_event/content_block_start",
          payload: message,
        },
      });
      return;
    ...
    if (event.type === "content_block_stop") {
      ...
      if (assistantBlock) {
        ...
        yield* completeAssistantTextBlock(context, assistantBlock, {
          rawMethod: "gemini/stream_event/content_block_stop",
          rawPayload: message,
        });
        return;
      }
  ...
  const handleAssistantMessage = Effect.fn("handleAssistantMessage")(function* (
    ...
    if (!context.turnState) {
      ...
      yield* offerRuntimeEvent({
        ...
        raw: {
          source: "claude.sdk.message",
          method: "gemini/synthetic-turn-start",
          payload: {},
        },
      });
    }
  ...
  const handleResultMessage = Effect.fn("handleResultMessage")(function* (
    ...
    if (status === "failed") {
      yield* emitRuntimeError(context, errorMessage ?? "Gemini turn failed.");
    }
  ...
  const handleSystemMessage = Effect.fn("handleSystemMessage")(function* (
    ...
    switch (message.subtype) {
      ...
      case "task_progress":
        if (message.usage) {
          const normalizedUsage = normalizeGeminiTokenUsage(
            message.usage,
      ...
      default:
        yield* emitRuntimeWarning(
          context,
          `Unhandled Gemini system message subtype '${message.subtype}'.`,
          message,
        );
        return;
    }
  ...
  const handleSdkMessage = Effect.fn("handleSdkMessage")(function* (
    ...
    switch (message.type) {
      ...
      default:
        yield* emitRuntimeWarning(
          context,
          `Unhandled Gemini SDK message type '${message.type}'.`,
          message,
        );
        return;
    }
  ...
  const runSdkStream = (context: GeminiSessionContext): Effect.Effect<void, Error> =>
    Stream.fromAsyncIterable(context.query, (cause) =>
      toError(cause, "Gemini runtime stream failed."),
    ).pipe(
  ...
  const handleStreamExit = Effect.fn("handleStreamExit")(function* (
    ...
    if (Exit.isFailure(exit)) {
      if (isGeminiInterruptedCause(exit.cause)) {
        if (context.turnState) {
          yield* completeTurn(
            context,
            "interrupted",
            interruptionMessageFromGeminiCause(exit.cause),
          );
        }
      } else {
        const message = messageFromGeminiStreamCause(exit.cause, "Gemini runtime stream failed.");
        yield* emitRuntimeError(context, message, Cause.pretty(exit.cause));
        yield* completeTurn(context, "failed", message);
      }
    } else if (context.turnState) {
      yield* completeTurn(context, "interrupted", "Gemini runtime stream ended.");
    }
  ...
  const stopSessionInternal = Effect.fn("stopSessionInternal")(function* (
    ...
    try {
      ...
      yield* emitRuntimeError(context, "Failed to close Gemini runtime query.", cause);
    }
  ...
  const startSession: GeminiAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      const contextRef = yield* Ref.make<GeminiSessionContext | undefined>(undefined);
      ...
      const canUseToolEffect = Effect.fn("canUseTool")(function* (
        ...
        if (!context) {
          return {
            behavior: "deny",
            message: "Gemini session context is unavailable.",
          } satisfies PermissionResult;
        }
      ...
      const geminiBinaryPath = geminiSettings.binaryPath;
      const modelSelection =
        input.modelSelection?.provider === "gemini" ? input.modelSelection : undefined;
      const caps = getGeminiModelCapabilities(modelSelection?.model);
      ...
      const queryOptions: GeminiQueryOptions = {
        ...(input.cwd ? { cwd: input.cwd } : {}),
        ...(apiModelId ? { model: apiModelId } : {}),
        pathToClaudeCodeExecutable: geminiBinaryPath,
        settingSources: [...GEMINI_SETTING_SOURCES],
      ...
      const queryRuntime = yield* Effect.try({
        try: () =>
          ...
          new ProviderAdapterProcessError({
            provider: PROVIDER,
            threadId,
            detail: toMessage(cause, "Failed to start Gemini runtime session."),
            cause,
          }),
      });
  ...
  const interruptTurn: GeminiAdapterShape["interruptTurn"] = Effect.fn("interruptTurn")(
  ...
  const readThread: GeminiAdapterShape["readThread"] = Effect.fn("readThread")(
    function* (threadId) {
      const context = yield* requireSession(threadId);
  ...
  const respondToRequest: GeminiAdapterShape["respondToRequest"] = Effect.fn("respondToRequest")(
  ...
  const stopSession: GeminiAdapterShape["stopSession"] = Effect.fn("stopSession")(
    function* (threadId) {
      const context = yield* requireSession(threadId);
      yield* stopSessionInternal(context, {
        emitExitEvent: true,
