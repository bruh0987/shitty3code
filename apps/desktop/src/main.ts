...
const APP_DISPLAY_NAME = isDevelopment ? "shitty3code (Dev)" : "shitty3code (Alpha)";
...
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "shitty3code (Dev)" : "shitty3code (Alpha)";
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("shitty3code failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `shitty3code ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/shitty3code (Alpha)` on Linux). This is
